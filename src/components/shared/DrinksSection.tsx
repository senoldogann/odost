...
export default function DrinksSection() {
  ...
  useEffect(() => {
    const fetchFeaturedDrinks = async () => {
      try {
        const response = await fetch(`/api/menu?type=BAARI&featured=true`)
        if (!response.ok) throw new Error('Korostettujen juomien haku epäonnistui')
        const data = await response.json()
        setItems(data)
      } catch (error) {
        console.error('Korostettujen juomien hakuvirhe:', error)
      } finally {
        setLoading(false)
      }
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {items.map((item) => (
            <div
              ...
              onClick={() => router.push('/baari/juomat')}
            >
